...
export default function TopBar({ onOpenMenu, spotlight }) {
  const { entities, activeEntityId, setActiveEntityId, activeEntity, loading, loadError, retry } = useEntity()
  const { profile, signOut } = useAuth()
  const [open, setOpen] = useState(false)
  const isLit = spotlight === 'entity-switcher'

  return (
    <header className="sticky top-0 z-30 flex items-center justify-between border-b border-lavender-200 bg-white/80 px-3 py-3 backdrop-blur sm:px-6 sm:py-3.5">
      <div className="flex items-center gap-2">
        <button onClick={onOpenMenu} className="rounded-lg p-1.5 text-ink-900 hover:bg-lavender-50 md:hidden">☰</button>
        <div className="relative">
          <button
            data-tutorial="entity-switcher"
            onClick={() => setOpen(o => !o)}
            className={`flex items-center gap-2 rounded-xl border border-lavender-200 bg-lavender-50 px-3 py-2 text-xs font-medium text-ink-900 transition-colors hover:bg-lavender-100 sm:px-4 sm:text-sm ${isLit ? 'tutorial-glow' : ''}`}
          >
          ...
          {open && (
            <div className="animate-fadeIn absolute left-0 top-12 z-20 w-72 rounded-xl2 border border-lavender-200 bg-white p-2 shadow-lg shadow-ink-900/5">
              ...
              {loading && <p className="px-3 py-2 text-xs text-ink-400">Memuat daftar PT...</p>}
              {!loading && loadError && (
                <div className="px-3 py-2 text-xs text-rose-500">
                  Gagal memuat: {loadError}
                  <button onClick={retry} className="ml-2 underline">Coba lagi</button>
                </div>
              )}
              {!loading && !loadError && entities.length === 0 && (
                <p className="px-3 py-2 text-xs text-ink-400">Belum ada PT terdaftar.</p>
              )}
              {entities.map(e => (
                <button
                  key={e.id}
