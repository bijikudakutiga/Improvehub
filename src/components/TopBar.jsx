...
export default function TopBar() {
  ...
  return (
    <header className="flex items-center justify-between border-b border-lavender-200 bg-white/80 px-6 py-3.5 backdrop-blur">
      <div className="relative">
        <button
          onClick={() => setOpen(o => !o)}
          className="flex items-center gap-2 rounded-xl border border-lavender-200 bg-lavender-50 px-4 py-2 text-sm font-medium text-ink-900 transition-colors hover:bg-lavender-100"
        >
          <span className="star-motif h-2.5 w-2.5 bg-lavender-500" />
          {activeEntity?.legal_name}
      ...
      <div className="flex items-center gap-4">
        <span className="text-sm text-ink-400">{profile?.full_name || 'Pengguna'}</span>
        <button onClick={signOut} className="rounded-lg border border-lavender-200 px-3 py-1.5 text-xs text-ink-400 hover:bg-lavender-50">
          Keluar
        </button>
      </div>
    </header>
